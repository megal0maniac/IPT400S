...
def writeToDB(weatherdata):
    ...
    currentdb = 'db/{}-{:02d}-{:02d}.db'.format(today.year, today.month, today.day)
...
def getFilename(timestamp):
    ...
    return 'db/{}-{:02d}-{:02d}.db'.format(today.year, today.month, today.day)
...
flag = 0
lasttime = int(time.time()) - resolution
while True:
    if ((int(time.time())%resolution == 0) and (lasttime != int(time.time()))):
        ser.flush()
